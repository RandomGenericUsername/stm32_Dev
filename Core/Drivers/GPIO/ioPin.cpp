...
bool ioPin::read(const bool &extrict) { return readIO(extrict); }
bool ioPin::write(const gpioState &state) { return writeIO(state); }
bool ioPin::toggle()
{
...
bool ioPin::readIO(const bool &extrict)
{
    if(_status != gpiostatusCode::ready)
    {
    ...
    return _instance->IDR & 0x1 << static_cast<paramType>(_pin);
}

bool ioPin::writeIO(const gpioState &state)
{
