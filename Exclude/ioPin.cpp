...
template<typename T>
paramIndex ioPin::assertParam(const T &param)
{
    if(std::is_same_v<const gpioPort &, decltype(param)>)
        return static_cast<paramIndex>(gpioParameters::port);
    else if(std::is_same_v<const gpioPin &, decltype(param)>)
        return static_cast<paramIndex>(gpioParameters::pin);
    else if(std::is_same_v<const gpioMode &, decltype(param)>)
        return static_cast<paramIndex>(gpioParameters::mode);
    else if(std::is_same_v<const gpioPUPD &, decltype(param)>)
        return static_cast<paramIndex>(gpioParameters::pupd);
    else if(std::is_same_v<const gpioOutputSpeed &, decltype(param)>)
        return static_cast<paramIndex>(gpioParameters::oSpeed);
    else if(std::is_same_v<const gpioOutputType &, decltype(param)>)
        return static_cast<paramIndex>(gpioParameters::oType);
    else if(std::is_same_v<const gpioState &, decltype(param)>)
        return static_cast<paramIndex>(gpioParameters::state);
    else if(std::is_same_v<const gpioFailSafe &, decltype(param)>)
        return static_cast<paramIndex>(gpioParameters::failSafe);
    else if(std::is_same_v<const gpioDebug &, decltype(param)>)
